...
docker start -d --name voting-app -p 5000:80 voting-app:latest

# Now create redis container
docker run -d --name redis

# Now stop voting app and then restart it using link tag with redis container
docker run -d -p 5000:80 --link redis:redis voting-app:latest

# Now pull docker image
docker pull postgres:9.4

# Now start worker node 
cd worker
docker build . -t worker-app

# Since worker app is build now use this command to start and link worker container with other two container
docker run -d --link redis:redis --link db:db worker-app:latest 

# Since python app is connected to redis
# And worker is connected with redis and postgres 
# Now create node container
